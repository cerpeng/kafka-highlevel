...
public class PropertiesParser {
    ...
    public PropertiesParser(String propsName) {
        this(propsName, null);
    }

    public PropertiesParser(String propsName, String charsetName) {
        // 如果是系统的绝对路径
        if (propsName.contains("/") || propsName.contains("\\")) {
            try {
                is = new FileInputStream(new File(propsName));
            } catch (FileNotFoundException e) {
                throw new RuntimeException(propsName + " file load error", e);
            }
        } else {
